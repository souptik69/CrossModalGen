...
from model.main_model_2 import Semantic_Decoder_AVVP_1, AV_VQVAE_Encoder, Semantic_Decoder_AVVP, AVT_VQVAE_Encoder
...
def train_epoch(Encoder, Decoder,ExpLogLoss_fn, train_dataloader, criterion, criterion_event, optimizer, epoch, total_step, args):
    ...
    for n_iter, batch_data in enumerate(train_dataloader):
        ...
        if (args.dataset_name == 'avvp_va'):
            ...
            audio_dim = audio_vq.size()[2]
            video_dim = video_vq.size()[2]
            audio_vq = audio_vq.reshape(-1, audio_dim)
            video_vq = video_vq.reshape(-1, video_dim)
            audio_class = Decoder(audio_vq)
            video_class = Decoder(video_vq)
