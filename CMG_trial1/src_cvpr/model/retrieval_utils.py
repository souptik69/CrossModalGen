...
def a2t(audio_embs, cap_embs, audio_cap_num, return_ranks=False):
    """Audio to Text retrieval - optimized version"""
    num_audios = len(audio_cap_num)
    ranks = np.zeros(num_audios)
    top1 = np.zeros(num_audios)
    mAP10 = np.zeros(num_audios)
    
    # Get unique audio embeddings (one per audio, not per caption)
    audio_indices = []
    start_idx = 0
    for i in range(num_audios):
        audio_indices.append(start_idx)
        start_idx += audio_cap_num[i]
    
    unique_audio_embs = audio_embs[audio_indices]
    
    # Compute full similarity matrix ONCE (num_audios x num_captions)
    print(f"Computing audio-to-caption similarity matrix ({num_audios} x {cap_embs.shape[0]})...")
    
    # Batch computation to avoid memory issues
    batch_size = 1000
    all_scores = []
    for i in tqdm(range(0, num_audios, batch_size), desc="a2t similarity"):
        batch_audio = torch.Tensor(unique_audio_embs[i:i+batch_size])
        batch_scores = util.cos_sim(batch_audio, torch.Tensor(cap_embs)).numpy()
        all_scores.append(batch_scores)
    
    scores_matrix = np.vstack(all_scores)  # (num_audios, num_captions)
    print(f"Similarity matrix computed. Shape: {scores_matrix.shape}")
    
    # Now compute ranks
    print("Computing ranks...")
    start_idx = 0
    for index in tqdm(range(num_audios), desc="a2t ranking"):
        ...
        # Computing mAP for the top 10 ranks
        relevant_inds = inds[:10]
        relevant_ranks = (relevant_inds >= start_idx) & (relevant_inds < start_idx + audio_cap_num[index])
        relevant_positions = np.where(relevant_ranks)[0] + 1  # 1-based indexing
        mAP10[index] = np.mean(1 / relevant_positions) if len(relevant_positions) > 0 else 0
...
def t2a(audio_embs, cap_embs, audio_cap_num, return_ranks=False):
    """Text to Audio retrieval - optimized version"""
    num_audios = len(audio_cap_num)
    
    # Get unique audio embeddings
    audio_indices = []
    start_idx = 0
    for i in range(num_audios):
        audio_indices.append(start_idx)
    ...
    unique_audio_embs = audio_embs[audio_indices]  # (num_audios, dim)
    
    # Compute full similarity matrix ONCE (num_captions x num_audios)
    print(f"Computing caption-to-audio similarity matrix ({cap_embs.shape[0]} x {num_audios})...")
    
    batch_size = 1000
    all_scores = []
    num_captions = cap_embs.shape[0]
    
    for i in tqdm(range(0, num_captions, batch_size), desc="t2a similarity"):
        batch_cap = torch.Tensor(cap_embs[i:i+batch_size])
        batch_scores = util.cos_sim(batch_cap, torch.Tensor(unique_audio_embs)).numpy()
        all_scores.append(batch_scores)
    
    scores_matrix = np.vstack(all_scores)  # (num_captions, num_audios)
    print(f"Similarity matrix computed. Shape: {scores_matrix.shape}")
    ...
    for index in tqdm(range(num_audios), desc="t2a ranking"):
        num_caps = audio_cap_num[index]
        
        for cap_i in range(num_caps):
            cap_idx = start_idx + cap_i
            scores = scores_matrix[cap_idx]
            inds = np.argsort(scores)[::-1]
            rank = np.where(inds == index)[0][0]
            ranks.append(rank)
            top1.append(inds[0])
