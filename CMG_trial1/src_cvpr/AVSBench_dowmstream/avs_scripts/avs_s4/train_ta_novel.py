...
from dataloader_novel import S4Dataset
    ...
    for epoch in range(args.max_epoches):
        for n_iter, batch_data in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                ...
                text_vq, vq = Encoder.Text_VQ_Encoder(text_feature)# [B, T, 768]
                

            text_vq = text_vq.reshape(-1, text_vq.shape[-1])
            output, visual_map_list, a_fea_list = model(imgs, text_vq.to(torch.float32))
        ...
        with torch.no_grad():
            for n_iter, batch_data in enumerate(val_dataloader):
                ...
                audio_vq, vq = Encoder.Audio_VQ_Encoder(audio_feature)# [B, T, 768]
            ...
            if miou > max_miou:
                model_save_path = os.path.join(log_dir, '%s_best.pth'%(args.session_name))
