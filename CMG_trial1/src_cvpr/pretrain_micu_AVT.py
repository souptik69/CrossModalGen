...
def main():
    ...
    parser.add_argument("--jigsaw_samples", type=int, default=2)# 8 256
    ...
    if args.dataset_name == 'vggsound_AVT_40k' or args.dataset_name == 'vggsound_AVT_90k':
    ...
    optimizer = torch.optim.Adam(chain(Text_ar_lstm.parameters(), \
                                       Encoder.parameters(), CPC.parameters(), Decoder.parameters(), jigsaw_cls.parameters()), lr=args.lr)
    ...
    for epoch in range(start_epoch+1, args.n_epoch):
        ...
        loss, total_step = train_epoch(CPC, Encoder,Text_ar_lstm, Audio_mi_net, Video_mi_net,Text_mi_net, Decoder, train_dataloader, criterion, criterion_event, 
                                       optimizer, optimizer_audio_mi_net, optimizer_video_mi_net, optimizer_text_mi_net, epoch, total_step, args, jigsaw_indices, jigsaw_cls, infonce)
...
def all_apply_mask(mask_prob, inputs1, inputs2, inputs3):
        mask = (torch.rand(inputs1.shape[:-1], device=inputs1.device) > mask_prob).double().cuda()
...
def compute_params(x, y, z):
    # 找出 x, y, z 中的第二大值，并设置 a, b, c 的值
    if (x >= y >= z) or (z >= y >= x):
        # y 是第二大值
        a = tanh_based_mapping(x / y)
        b = 0.3
        c = tanh_based_mapping(z / y)
    elif (y >= x >= z) or (z >= x >= y):
        # x 是第二大值
        a = 0.3
        b = tanh_based_mapping(y / x)
        c = tanh_based_mapping(z / x)
    elif (x >= z >= y) or (y >= z >= x):
        # z 是第二大值
        a = tanh_based_mapping(x / z)
        b = tanh_based_mapping(y / z)
        c = 0.3
    else:
...
def train_epoch(CPC,Encoder,Text_ar_lstm, Audio_mi_net, Video_mi_net, Text_mi_net, Decoder,train_dataloader, criterion, criterion_event, optimizer, optimizer_audio_mi_net, optimizer_video_mi_net, optimizer_text_mi_net, epoch, total_step, args, jigsaw_indices, jigsaw_cls, infonce):
    global global_steps
    ...
    m_a_ratio, m_v_ratio, m_t_ratio = 0.3, 0.3, 0.3
    last_score = 0
    ...
    for n_iter, batch_data in enumerate(train_dataloader):
        ...
        lld_audio_loss, lld_video_loss, lld_text_loss = torch.zeros(1).cuda(),torch.zeros(1).cuda(),torch.zeros(1).cuda()
        # optimizer_audio_mi_net, lld_audio_loss, optimizer_video_mi_net, lld_video_loss, optimizer_text_mi_net, lld_text_loss  =\
        #     mi_first_forward(Audio_mi_net, Video_mi_net, Text_mi_net, optimizer_audio_mi_net,optimizer_video_mi_net,optimizer_text_mi_net,
        #                      audio_encoder_result.detach(), video_club_feature.detach(), text_encoder_result.detach(),
        #                      audio_vq.detach(), video_vq.detach(), text_vq.detach(), mi_iters)
        ...
        m_a_s, m_v_s, m_t_s, _,_,_ = all_apply_mask(m_a_ratio, audio_semantic_result, video_semantic_result, text_semantic_result)
        
        for i in range(batch_dim):
            infonce_fine_loss += infonce(m_a_s[i], audio_semantic_result[i]) + infonce(m_a_s[i], video_semantic_result[i]) + infonce(m_a_s[i], text_semantic_result[i]) + \
                                infonce(m_v_s[i], audio_semantic_result[i]) + infonce(m_v_s[i], video_semantic_result[i]) + infonce(m_v_s[i], text_semantic_result[i]) + \
                                infonce(m_t_s[i], audio_semantic_result[i]) + infonce(m_t_s[i], video_semantic_result[i]) + infonce(m_t_s[i], text_semantic_result[i]) 
        
        infonce_fine_loss /= batch_dim
        
        infonce_coarse_loss = infonce(torch.mean(m_a_s,dim=1), torch.mean(audio_semantic_result,dim=1)) + infonce(torch.mean(m_a_s,dim=1), torch.mean(video_semantic_result,dim=1)) + infonce(torch.mean(m_a_s,dim=1), torch.mean(text_semantic_result,dim=1)) + \
                            infonce(torch.mean(m_v_s,dim=1), torch.mean(audio_semantic_result,dim=1)) + infonce(torch.mean(m_v_s,dim=1), torch.mean(video_semantic_result,dim=1)) + infonce(torch.mean(m_v_s,dim=1), torch.mean(text_semantic_result,dim=1)) + \
                            infonce(torch.mean(m_t_s,dim=1), torch.mean(audio_semantic_result,dim=1)) + infonce(torch.mean(m_t_s,dim=1), torch.mean(video_semantic_result,dim=1)) + infonce(torch.mean(m_t_s,dim=1), torch.mean(text_semantic_result,dim=1))
        
        
        # 调整mask_ratio
        score_a_f, score_v_f, score_t_f, score_a_c, score_v_c, score_t_c = torch.zeros(1).cuda(), torch.zeros(1).cuda(), torch.zeros(1).cuda(), torch.zeros(1).cuda(), torch.zeros(1).cuda(), torch.zeros(1).cuda()
        for i in range(batch_dim):
            score_a_f += torch.sum(torch.diag(torch.matmul(m_a_s[i], audio_semantic_result[i].T))) + \
                         ...
                         torch.sum(torch.diag(torch.matmul(m_a_s[i], text_semantic_result[i].T))) 
            ...
            score_v_f += torch.sum(torch.diag(torch.matmul(m_v_s[i], audio_semantic_result[i].T))) + \
                         ...
                         torch.sum(torch.diag(torch.matmul(m_v_s[i], text_semantic_result[i].T))) 
                         
            score_t_f += torch.sum(torch.diag(torch.matmul(m_t_s[i], audio_semantic_result[i].T))) + \
                         torch.sum(torch.diag(torch.matmul(m_t_s[i], video_semantic_result[i].T))) + \
                         torch.sum(torch.diag(torch.matmul(m_t_s[i], text_semantic_result[i].T))) 
        
        seq_len = audio_semantic_result.shape[1]
        
        # 除以batch_dim，再针对单个相似度除以seq_len
        score_a_f /= (batch_dim*seq_len)
        score_v_f /= (batch_dim*seq_len)
        score_t_f /= (batch_dim*seq_len)
        ...
        score_a_c = torch.sum(torch.diag(torch.matmul(torch.mean(m_a_s,dim=1), torch.mean(audio_semantic_result,dim=1).T))) + \
                         ...
                         torch.sum(torch.diag(torch.matmul(torch.mean(m_a_s,dim=1), torch.mean(text_semantic_result,dim=1).T))) 
        ...
        score_v_c = torch.sum(torch.diag(torch.matmul(torch.mean(m_v_s,dim=1), torch.mean(audio_semantic_result,dim=1).T))) + \
                         ...
                         torch.sum(torch.diag(torch.matmul(torch.mean(m_v_s,dim=1), torch.mean(text_semantic_result,dim=1).T))) 
                         
        score_t_c = torch.sum(torch.diag(torch.matmul(torch.mean(m_t_s,dim=1), torch.mean(audio_semantic_result,dim=1).T))) + \
                         torch.sum(torch.diag(torch.matmul(torch.mean(m_t_s,dim=1), torch.mean(video_semantic_result,dim=1).T))) + \
                         torch.sum(torch.diag(torch.matmul(torch.mean(m_t_s,dim=1), torch.mean(text_semantic_result,dim=1).T))) 

        # 针对单个相似度除以batch_dim（因为个数是batch_dim）
        score_a_c /= batch_dim
        score_v_c /= batch_dim
        score_t_c /= batch_dim
        ...
        # audio_emd, video_emd, text_emd = audio_semantic_result.mean(dim=1),video_semantic_result.mean(dim=1),text_semantic_result.mean(dim=1)
        audio_emd, video_emd, text_emd = audio_vq.reshape(-1,emd_dim),video_vq.reshape(-1,emd_dim),text_vq.reshape(-1,emd_dim)
        ...
        text_parts = torch.split(text_emd, text_emd.shape[1] // args.jigsaw_num_splits, dim=1)

        parts = ()
        for i in range(args.jigsaw_num_splits):
            random_idx = random.randint(0, 2)
            if random_idx == 0:
                parts += (audio_parts[i],)  # 将 audio_parts[i] 包裹在 tuple 中
            elif random_idx == 1:
                parts += (video_parts[i],)  # 将 video_parts[i] 包裹在 tuple 中
            else:
                parts += (text_parts[i],)  # 将 text_parts[i] 包裹在 tuple 中
        ...
        combinations = torch.cat(combinations, dim=0) # torch.Size([10240, 256]) [bs*jigsaw_indices_len ,modal_num*embedding_dim]
        
        jigsaw_labels = torch.cat(jigsaw_labels, dim=0).squeeze(1).type(torch.LongTensor).cuda()#torch.Size([10240])
        # import pdb
        # pdb.set_trace()
        ...
        """oscmg best model wo[cpc,cmcm,mi]"""
...
def mi_first_forward(Audio_mi_net, Video_mi_net, Text_mi_net, optimizer_audio_mi_net,optimizer_video_mi_net,optimizer_text_mi_net,
                             audio_encoder_result, video_club_feature, text_encoder_result,
                             audio_vq, video_vq, text_vq, mi_iters):
    ...
    for i in range(mi_iters):
        ...
        # 视频的处理与音频、文本模态不同是因为考虑到视频更复杂，特征提取难度更大
...
def mi_second_forward(CPC, audio_feature, video_feature, text_feature, Encoder, Audio_mi_net, Video_mi_net, Text_mi_net, Decoder,epoch):
    ...
    # mi_video_loss, mi_text_loss, mi_audio_loss = torch.zeros(1).cuda(),torch.zeros(1).cuda(),torch.zeros(1).cuda()
    ...
    # accuracy1, accuracy2, accuracy3, accuracy4, \
    # accuracy5, accuracy6, accuracy7, accuracy8, accuracy9, \
    # cpc_loss = torch.zeros(1).cuda(), torch.zeros(1).cuda(), torch.zeros(1).cuda(),torch.zeros(1).cuda(),torch.zeros(1).cuda(), \
    ...
    return audio_embedding_loss, video_embedding_loss, text_embedding_loss, mi_audio_loss, mi_video_loss, mi_text_loss, \
           ...
           audio_recon_loss, video_recon_loss, text_recon_loss, audio_class, video_class, text_class, cmcm_loss, equal_num
