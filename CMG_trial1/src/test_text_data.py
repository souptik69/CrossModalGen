...
print("=" * 80)
print("ANALYZING PROMPTS WITH transformers LIBRARY")
print("=" * 80)
print(f"\nTotal tokens in cnt.pkl: {len(id2idx)}")
print(f"Sample tokens from cnt.pkl: {list(id2idx.items())[:10]}")
...
# Output file
output_file = '/project/ag-jafra/Souptik/VGGSoundAVEL/CMG/prompt_analysis_transformers.txt'

with open(output_file, 'w') as f:
    f.write("=" * 80 + "\n")
    f.write("PROMPT ANALYSIS WITH transformers LIBRARY\n")
    f.write("=" * 80 + "\n\n")
    f.write(f"Total tokens in cnt.pkl: {len(id2idx)}\n\n")
    ...
    for csv_path, dataset_name in csv_files:
        ...
        for idx, row in df.iterrows():
            ...
            f.write(f"All tokens: {', '.join(non_special_tokens)}\n")
            ...
            # Print to console
            print(f"\nLabel: {label}")
            print(f"  Kept: {len(words_kept)} words - {words_kept[:5]}{'...' if len(words_kept) > 5 else ''}")
            print(f"  Filtered: {len(words_filtered)} words - {words_filtered[:5]}{'...' if len(words_filtered) > 5 else ''}")
        
        # Summary for this dataset
        summary = f"\n{dataset_name} SUMMARY:\n"
        summary += f"Total words kept: {total_words_kept}\n"
        summary += f"Total words filtered: {total_words_filtered}\n"
        summary += f"Keep ratio: {total_words_kept / (total_words_kept + total_words_filtered) * 100:.2f}%\n"
