...
path_checkpoints = "/project/ag-jafra/Souptik/CMG_New/Experiments/CMG_trial1/Models/Novel_AV_final_2/Meta_Hier/checkpoint/DCID-model-5.pt"
...
hier_weights_parameter = quantizer.hier_weights
modal_weights_video = modal_weights[:, 0:2]
modal_weights_audio = modal_weights[:, 2:4]

print("="*60)
print("Model Weights Analysis - First 10 Vectors")
print("="*60)

for i in range(400):  # Print first 10 vectors
    print(f"Vector {i:2d}:")
    print(f"  Video modality weights: [{modal_weights_video[i,0]:.4f}, {modal_weights_video[i,1]:.4f}]")
    print(f"  Audio modality weights: [{modal_weights_audio[i,0]:.4f}, {modal_weights_audio[i,1]:.4f}]")
