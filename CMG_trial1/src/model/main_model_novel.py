...
class AV_VQVAE_Encoder(nn.Module):
    ...
    def forward(self, audio_feat, video_feat, epoch):
        ...
        return audio_encoder_result, video_spatial, \
...
class Cross_VQEmbeddingEMA_AV(nn.Module):
    def __init__(self, n_embeddings, embedding_dim, commitment_cost=0.25, decay=0.99, epsilon=1e-5):
        ...
        # self.register_buffer("modal_weights", torch.tensor([0.8, 0.2, 0.2, 0.8]))
        # self.register_buffer("hier_weights", torch.tensor([0.1, 0.1]))
        # log_ratio = math.log(0.8/0.2)  # ≈ 1.3863
        # self.modality_weights = nn.Parameter(torch.tensor([log_ratio, 0, 0, log_ratio]))
        # logit_01 = math.log(0.1/(1-0.1))  # ≈ -2.2
        # self.hierarchical_weights = nn.Parameter(torch.tensor([logit_01, logit_01]))
    ...
    def forward(self, audio_semantic, video_semantic, epoch):
        ...
        # self.modal_weights[0:2] = F.softmax(self.modality_weights[0:2], dim=0)  # Video segment weights
        # self.modal_weights[2:4] = F.softmax(self.modality_weights[2:4], dim=0)  # Audio segment weights
        # self.hier_weights = torch.sigmoid(self.hierarchical_weights)
        ...
        hier_weights = torch.sigmoid(self.hierarchical_weights)

        # self.modal_weights[:, 0:2] = F.softmax(self.modality_weights[:, 0:2], dim=1)  # Video segment weights for all vectors
        # self.modal_weights[:, 2:4] = F.softmax(self.modality_weights[:, 2:4], dim=1)  # Audio segment weights for all vectors
        # self.hier_weights = torch.sigmoid(self.hierarchical_weights)
        ...
        if self.training:
            ...
            # hier_weights = self.hier_weights    

            self.ema_count = self.decay * self.ema_count + (1 - self.decay) * (
                torch.sum(v_encodings, dim=0) + torch.sum(a_encodings, dim=0)
            ) / 2
            ...
            # self.ema_weight[:, :D] = self.decay * self.ema_weight[:, :D] + (1 - self.decay) * (
            #     modal_weights[0] * v_dw + modal_weights[1] * a_dw_v
            # )
            ...
            # self.ema_weight[:, D:] = self.decay * self.ema_weight[:, D:] + (1 - self.decay) * (
            #     modal_weights[2] * v_dw_a + modal_weights[3] * a_dw
            # )
            

            # STEP 2: SEGMENT ALIGNMENT WITH InfoNCE LOSS

            video_segments = self.embedding[:, :D]  # [M, D]
            audio_segments = self.embedding[:, D:]  # [M, D]
            
            video_segments_norm = F.normalize(video_segments, p=2, dim=1)  # [M, D]
            audio_segments_norm = F.normalize(audio_segments, p=2, dim=1)  # [M, D]
            ...
            segment_loss = torch.mean(-positive_logits + logsumexp_logits)
  
            # segment_losses = []
            # for i in range(M):
            #     logits = similarity_matrix[i]  # [M]
                
            #     positive_logit = logits[i]
 
            #     loss_i = -positive_logit + torch.logsumexp(logits, dim=0)
            #     segment_losses.append(loss_i)
            # segment_loss = torch.stack(segment_losses).mean()


            # STEP 3: HIERARCHICAL INFLUENCE BETWEEN SEGMENTS
            
            if not hasattr(self, 'v2a_projection') or self.v2a_projection.weight.size(0) != D:
                self.v2a_projection = nn.Linear(D, D, dtype=torch.float64).to(self.embedding.device)
                self.a2v_projection = nn.Linear(D, D, dtype=torch.float64).to(self.embedding.device)

                nn.init.xavier_normal_(self.v2a_projection.weight, gain=0.01)
                nn.init.xavier_normal_(self.a2v_projection.weight, gain=0.01)
                nn.init.zeros_(self.v2a_projection.bias)
                nn.init.zeros_(self.a2v_projection.bias)

            # v2a_influence = self.v2a_projection(self.embedding[:, :D])
            # a2v_influence = self.a2v_projection(self.embedding[:, D:])
            ...
            with torch.no_grad(): 
                ...
                self.ema_weight[:, D:] += self.hier_weights[:, 0].unsqueeze(1) * v2a_influence
                self.ema_weight[:, :D] += self.hier_weights[:, 1].unsqueeze(1) * a2v_influence

            # self.ema_weight[:, D:] += self.hier_weights[:, 0].unsqueeze(1) * v2a_influence
            # self.ema_weight[:, :D] += self.hier_weights[:, 1].unsqueeze(1) * a2v_influence
            
            div_epsilon = 1e-5
            self.embedding = self.ema_weight / (self.ema_count.unsqueeze(-1) + div_epsilon)
